...
count = 0
# 设置字典中需要保留的字段
keys_to_keep = ['local_batch_name', 'min', 'type', 'min_section', "school_id", "province_id", 'proscore',
                'year']
# proid为我国34个省对应id的列表
...
for sid in range(30, 3681):
    for pid in proid:
        for yid in range(2018, 2023):
            # url根据年份、省份、学校代号的不同，会返回相应学校的json文件
            url = "https://static-data.gaokao.cn/www/2.0/schoolprovincescore/{0}/{1}/{2}.json".format(sid, yid, pid)
    ...
    count += 1
    print(count)
# 将out_to_file格式转换为json
out_to_file = json.dumps(out_to_file, indent=4)
# 将Unicode编码转换为中文
out_to_file = out_to_file.encode('utf-8').decode('unicode_escape')
# 写入到本地文件中
with open("score_province.json", 'w', encoding='utf-8') as f:
    f.write(out_to_file)
